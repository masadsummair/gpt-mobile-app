...
export const askQuestion = createAsyncThunk(
    ...
    async (question, { rejectWithValue, dispatch, getState }) => {
        try {
            const { chatSlice: { chat, selectedThreat }, userSlice: { user } } = getState();
            let context = [];
            context.push({
                "role": "system",
            ...
            if (chat[selectedThreat][1].length > 1) {

                const arrayLength = chat[selectedThreat][1].length;
                for (let i = 0; i < arrayLength - 2; i++) {
                    const item = chat[selectedThreat][1][i];
                    if (item.type == "answer") {
            ...
            if (chat[selectedThreat][1].slice(0, -2).length == 0) {
                ...
                await firestore().collection('chat').doc(user.id).collection("threat").doc(chat[selectedThreat][0].id).set({
                    chatMessages: [...chat[selectedThreat][1].slice(0, -2), { type: "question", message: question }, { type: "answer", message: result.result, like: true }],
                    createdAt: new Date(),
                });
                return {
                    new: false,
